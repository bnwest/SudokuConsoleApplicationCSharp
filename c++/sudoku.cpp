...
//  "100000000059102000007800029005300604000060000304008500510004800000201950000000007"
  "563700000002000947040100000030050209020000080409010050000004010254000600000006495" // locked candidates
...
void
convertInput(const char *strInput)
{
    char strLine[16];
    const char *strStart;
    char strCell[2];
    ...
    for (i = 0; i<9; i++) {
        ...
        //strncpy_s(strLine, 16, strStart, 9);
        for (j = 0; j<9; j++) {
            strCell[0] = strLine[j];
            strCell[1] = '\0';
    ...
    for (i = 0; i<9; i++) {
        ...
        if ( iNumPairs > 0 ) {
            for (k = 0; k<iNumPairs; k++) {
                printf("found candidate pair( %d %d ) at cell(%d,%d)\n",
                    Pairs[k][2] + 1, Pairs[k][3] + 1,
        ...
        if ( iNumPairs > 1 ) {
            for (ii = 0; ii<iNumPairs - 1; ii++) {
                for (jj = ii + 1; jj<iNumPairs; jj++) {
                    ...
                    if ( foundMatch ) {
                        printf("found a pair( %d %d ) at cell(%d,%d) and cell(%d,%d) row %d\n",
        ...
        if ( iNumHiddenPairs > 0 ) {
            for (j = 0; j<9; j++) {
                ...
                for (hp = 0; hp<iNumHiddenPairs; hp++) {
                    j1 = HiddenPairs[hp][1];
                    j2 = HiddenPairs[hp][3];
                    bool lookingAtPairCell = (j == j1 || j == j2);
